...
const seriesCtrl = {
    ...
    createSeries: async (req, res) => {
        try {
            ...
            if (!image && !banner && !title && !free && !description && !donation && !price && !seasons) return res.status(400).json({ msg: "All payload are required"})
    ...
    getSingleSeries: async (req, res) => {
        try {
            const series = await Series.findById({ _id: req. params.id}).populate("seasons")

            if (!series) return res.status(400).json({ msg: "Series does not exist"})
